...
生成日期：2026-09-21。检查基线：`bfe4a554668e1479f5cd746e003ba55dbede9c5a`（默认分支 `main`）；含本次整理提交的增量。

用途：高维多模态多目标算法/测试集。

阅读顺序：README → source/ → ARCHIVE_INDEX.md/json。

本清单覆盖 **34 个文件路径**。逐文件路径、类型、历史范围、字节数、Git 内容哈希和适用检查见 [FILEMAP.csv](FILEMAP.csv)。
...
| 目录 | 文件数 | 主要类型 |
|---|---:|---|
| (根目录) | 8 | 说明文档, navigation-index, 原始归档包 |
| [source](<source/>) | 26 | 源码 |
